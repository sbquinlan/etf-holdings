...
export class Channel<T> {
  ...
  private head: ChannelNode<IteratorResult<T>, void> = new ChannelNode<IteratorResult<T>, void>();
  constructor() {
  ...
  private *gen(node: ChannelNode<IteratorResult<T>, void> | undefined) {
    do {
      yield node!.value;
      node = node!.next ?? (node!.next = this.sealed ? undefined : new ChannelNode<IteratorResult<T>, void>());
    } while (node)
  }
